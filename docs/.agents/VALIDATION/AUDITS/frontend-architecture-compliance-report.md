...
| 15 | Models page renders two lifecycle surfaces (Runtime + Lifecycle) | HIGH | Architectural violation | Lifecycle is a runtime concern; duplicate Install/Start/Stop/Update/Remove + Activate/Deactivate buttons | **RESOLVED 2026-06-08** by `runtime-canonical-models-page` feature (single control surface in the Runtime Section; legacy Lifecycle block removed; `useModelLifecycleAction` no longer imported by the page). See [`../../SPECS/FEATURES/runtime-canonical-models-page/audits/models-page-canonical-control-surface.md`](../../SPECS/FEATURES/runtime-canonical-models-page/audits/models-page-canonical-control-surface.md). |
| 16 | Models page falls back to `useModels()` instead of single composed-view source | MEDIUM | Architectural debt | Backend `/api/models/with-runtimes` returns the catalog with empty `runtimes[]` for non-migrated models; the legacy fallback was redundant | **RESOLVED 2026-06-08** — page now depends solely on `useModelsWithRuntimes()`; pre-removal terminal check confirmed backend coverage. |
