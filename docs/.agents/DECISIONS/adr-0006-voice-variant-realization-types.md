...
> (`processing`, `stale`, `ready`, `failed`) is **superseded** by [ADR-0008](adr-0008-voice-variant-build-lifecycle.md).
...
> [ADR-0010](adr-0010-voice-source-assets-and-automatic-variant-provisioning.md): realizations build
...
> a voice) is **orthogonal** to a Voice's *origin* ([ADR-0011](adr-0011-voice-creation-sources.md)
...
1. **Realization never leaks.** No `/v1` response, Voice ID, Voice Library, or marketplace
   ...
   `public_voice_id` + `model` only ([ADR-0004](adr-0004-voice-variant-model-separation.md)).
...
- **Negative / costs:** adapters must set an accurate `artifact_type`; the taxonomy needs light
  ...
  realization — upheld by review + the boundary in [Runtime §6](../ARCHITECTURE/runtime-architecture.md).
- **Follow-ups:** `REALIZATION_TYPES` constant + validation helper; adapters reference it
  ...
  [ADR-0001](adr-0001-voice-variant-split.md); orthogonal to capabilities
  ([ADR-0003](adr-0003-model-capability-contract.md)) and edition availability
