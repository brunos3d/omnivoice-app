...
PeakVox is a model-agnostic [Universal Voice Runtime](../CONTEXT/VISION.md): many providers
(OmniVoice, OmniVoice Singing, Fish Audio, Kokoro, OpenVoice, and future engines) plug in
behind one [`ModelAdapter`](../ARCHITECTURE/runtime-architecture.md) contract. These models differ in
...
requirements/license/provider metadata of [ADR-0002](adr-0002-model-as-first-class-entity.md).)
...
- **API validation:** the public API maps a capability mismatch to a stable `422` with a
  ...
  the stable API surface ([API §3](../ARCHITECTURE/api-architecture.md)).
...
- **Follow-ups:** Phase 2 adds the superset + `capability_version` to `ModelCapabilities`
  ...
  ([Runtime §6](../ARCHITECTURE/runtime-architecture.md)); UI/API/Marketplace consume it in their
  respective phases. Ties to [ADR-0001](adr-0001-voice-variant-split.md) (a variant's existence for
  a model is orthogonal to the model's capabilities) and [ADR-0004](adr-0004-voice-variant-model-separation.md)
