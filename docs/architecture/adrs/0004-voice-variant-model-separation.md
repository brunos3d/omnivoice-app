...
PeakVox is a [Universal Voice Runtime](../00-VISION.md) where **voices are portable assets**
...
OmniVoice-specific artifacts). [ADR-0001](0001-voice-variant-split.md) split identity from
realization, and [ADR-0002](0002-model-as-first-class-entity.md) made Model first-class. This
...
([Runtime §4](../10-RUNTIME_ARCHITECTURE.md)).
...
3. **No marketplace feature is tied to a specific provider.** Listings reference Voices;
   ...
   ([ADR-0003](0003-model-capability-contract.md)), never hard-coded to a provider.
...
- **Positive:** model coupling is structurally prevented; Voice IDs are durable public
  ...
  new [model categories](../10-RUNTIME_ARCHITECTURE.md#12-model-classification)) integrate
...
- **Negative / costs:** requires ongoing discipline in reviews (reject any PR that leaks
  ...
  on every generation.
- **Follow-ups:** enforced by [Domain §1–§5](../02-DOMAIN_ARCHITECTURE.md), the
  [Data model](../03-DATA_ARCHITECTURE.md) (separate `voices` / `voice_variants` / `models`
  tables), the [API contract](../04-API_ARCHITECTURE.md), the
  [Runtime](../10-RUNTIME_ARCHITECTURE.md), and the
  [Marketplace](../05-MARKETPLACE_ARCHITECTURE.md). Builds directly on
  [ADR-0001](0001-voice-variant-split.md) and [ADR-0002](0002-model-as-first-class-entity.md);
  capabilities-belong-to-Model is governed by [ADR-0003](0003-model-capability-contract.md).
