...
> See also: [Data](data-architecture.md) · [Roadmap](../ROADMAP/ROADMAP.md) ·
> [ADR-0001](../DECISIONS/adr-0001-voice-variant-split.md)
...
`voice_variants` (per-model artifacts). See [ADR-0001](../DECISIONS/adr-0001-voice-variant-split.md).
...
- **Feature flags:** introduce `settings.features` derived from `settings.EDITION`
  ([Product §4.1](product-architecture.md)). CE = all commercial flags off. This is a config
...
- **Schema-ready commercial tables** (`creators`, `marketplace_listings`, `credit_ledgers`,
  ...
  editions but written only when their flag is on ([Data §4](data-architecture.md)).
...
variant pins. See [ADR-0002](../DECISIONS/adr-0002-model-as-first-class-entity.md).
...
Recommended order (also reflected in the [Roadmap](../ROADMAP/ROADMAP.md)):
