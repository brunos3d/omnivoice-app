...
- **Extends:** [ADR-0006](adr-0006-voice-variant-realization-types.md) (realization types),
  [ADR-0008](adr-0008-voice-variant-build-lifecycle.md) (build lifecycle),
  [ADR-0009](adr-0009-artifact-versioning-and-retention.md) (artifact versioning)
...
- **Generalized by:** [ADR-0011](adr-0011-voice-creation-sources.md) — the **Voice Source Asset** is
...
Validation of the Universal Voice Runtime ([11-PHASE-1-RETROSPECTIVE](../VALIDATION/RETROSPECTIVES/phase-1-retrospective.md),
[12-PROVIDER-VALIDATION](../VALIDATION/PROVIDER_VALIDATIONS/provider-validation.md)) surfaced a gap that the existing ADRs
...
different realization format ([ADR-0006](adr-0006-voice-variant-realization-types.md)):

| Provider | Realization | Built from |
|---|---|---|
| OmniVoice / OmniVoice Singing | `reference_sample` | the original audio |
| Fish Audio | `speaker_embedding` (assumed — see [12](../VALIDATION/PROVIDER_VALIDATIONS/provider-validation.md) §3.2) | the original audio |
...
variant from that source — i.e. the adapter's `supported_realization_types` ([ADR-0008](adr-0008-voice-variant-build-lifecycle.md))
can be produced from the Source Asset, the model is installed/active, and it is available in the
edition ([ADR-0005](adr-0005-edition-scoped-model-availability.md)). Preset-only providers (Kokoro,
...
- A Voice that has *no* compatible installed model simply has zero variants — a valid identity
  with no realizations (consistent with [Domain §5](../ARCHITECTURE/domain-architecture.md) invariants).
- Preset voices remain a separate origin (the `Voice.is_preset_voice` hook); they are the
  **`PRESET_VOICE`** Creation Source in [ADR-0011](adr-0011-voice-creation-sources.md), which
  generalizes voice origin beyond the Source Asset
  ([12 §4.4](../VALIDATION/PROVIDER_VALIDATIONS/provider-validation.md)).
...
- **Positive:**
  - A Voice is finally, formally, *not* a WAV file — the Source Asset is a distinct canonical
    layer, and the public identity is fully model-independent (upholds [Vision](../CONTEXT/VISION.md)).
...
- **Follow-ups / what this enables:**
  ...
  - **[ADR-0011](adr-0011-voice-creation-sources.md):** Voice Creation Sources — generalizes voice
  ...
  - Auto-routing ([Vision §Future](../CONTEXT/VISION.md), [12 §8](../VALIDATION/PROVIDER_VALIDATIONS/provider-validation.md)) gains a
