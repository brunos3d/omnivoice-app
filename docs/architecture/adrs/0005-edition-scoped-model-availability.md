...
PeakVox is model-agnostic ([ADR-0004](0004-voice-variant-model-separation.md)), but **not every
...
- **`Model.editions`** (existing JSON list, e.g. `["community"]`, `["community","cloud"]`) is the
  ...
  editions`.
- **`ModelLicense`** ([ADR-0002](0002-model-as-first-class-entity.md)) records the licensing
...
- **Follow-ups:** the Runtime availability check + `ModelNotAvailableInEdition` (Phase 3.8); the
  ...
  edition availability ([Product §3](../01-PRODUCT_ARCHITECTURE.md)). Ties to
  [ADR-0003](0003-model-capability-contract.md) (capabilities) — availability and capability are
