...
> See also: [Product](product-architecture.md) · [Data §5](data-architecture.md) ·
> [Migration](migration-architecture.md) · existing [SAAS_ARCHITECTURE](../ARCHIVE/LEGACY/SAAS_ARCHITECTURE.md)
...
not a domain change. See [Migration §Auth](migration-architecture.md).
...
[Runtime](runtime-architecture.md) + adapters** as Community Edition — there is no separate
Cloud inference codebase; only the execution substrate differs ([Runtime §9.2](runtime-architecture.md)).
...
- **Scaling:** workers scale on queue depth; GPU pools per model class. See
  [Roadmap — Phase 10 (Production Scaling)](../ROADMAP/ROADMAP.md).
...
- **Postgres** (primary + read replicas) replaces SQLite for Cloud; **Alembic** replaces the
  idempotent startup runner. Models are portable (see [Data §5](data-architecture.md)).
...
([Product §4.2](product-architecture.md)). This keeps the open-core license boundary clean
