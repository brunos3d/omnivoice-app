...
1. **The contract is `Voice` + `Model`, never artifacts.** Callers reference a stable
   `public_voice_id` and a `model` id; the [Runtime](runtime-architecture.md) resolves the
...
2. **No model internals on the wire.** Embeddings, checkpoints, training/variant formats, and
   ...
   ([ADR-0004](../DECISIONS/adr-0004-voice-variant-model-separation.md)). The public surface is
...
6. **`model` may be a specific id, omitted (default), or `"auto"`** (future runtime routing) —
   the same endpoint, unchanged client code. See [Vision §Future](../CONTEXT/VISION.md) and
   [Runtime §7](runtime-architecture.md).
...
resolve voice  → Voice by public_voice_id  (+ authorize: owned | public | marketplace)
  ▼
   ...
   ├─ ready    → resolve active artifact version ([ADR-0009](../DECISIONS/adr-0009-artifact-versioning-and-retention.md)) → generate
...
- **Key namespace:** `pv_live_…` / `pv_test_…` (renamed from `ov_live_…`; old prefix accepted
  during transition — see [Migration](migration-architecture.md)). Hashed (sha256), display
...
- Quotas/credits are checked **before** reserving inference; royalties accrue **after**
  success. See [Monetization](monetization-architecture.md).
